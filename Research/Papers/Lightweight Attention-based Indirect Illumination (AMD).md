...
**2026-09-18 补记（传言级，待核实）**：GPU 爆料人 Kepler_L2 称 AMD 正在为下一代游戏硬件开发对标 NVIDIA DLSS 5 的神经渲染方案，内部代号 **"Neural Lighting（神经光照）"**，可能限定在 **RDNA 5** 架构运行（复杂神经渲染模型算力需求高），并称其**除屏幕空间信息外还会调用引擎数据（含相机视野外的几何体）**，落在 RDNA 5 的 Radeon RX 10000 / PS6 / Xbox Project Helix 硬件世代。报道方明确把本文（GPUOpen《基于轻量级注意力机制的间接光照》，即**用 RSM 引入屏幕外几何**）列为可能的技术路径参照。

> **这条链路值得记住：** 本文的立项理由就是"screen-space 拿不到屏幕外的光"，而**它与 DLSS 5 的路线差异恰好也在这里**——DLSS 5 是通用屏幕空间模型（基于已渲染画面做调整），AMD 传闻路线要吃引擎侧几何。**"屏幕空间 vs 引擎感知"可能成为下一代神经渲染的阵营分界线。**
>
> 可信度：爆料 + 二手转载，**未经 AMD 官方确认**；记入 Watchlist，等官方口径或 SDK 证据。
